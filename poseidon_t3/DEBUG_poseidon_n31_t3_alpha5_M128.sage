...
round_constants = ['0x6630420a', '0x6e77b6b5', '0x085badf3', '0x06d17b3c', '0x2d2ca8bb', '0x3e585d13', '0x7f96d29c', '0x02c60c89', '0x3d73c75d', '0x44685392', '0x7545b9b7', '0x14abb4e4', '0x18bc8537', '0x204f93c1', '0x4081b364', '0x5cbe9f9b', '0x7f9f90a1', '0x3cbf7bc2', '0x268a6eab', '0x6f243411', '0x2c7554f8', '0x5fab2c61', '0x111376ae', '0x2cfe0ebe', '0x36b2e729', '0x29909ad3', '0x28b820d6', '0x54323e5d', '0x786e4d2e', '0x324f30d3', '0x65f93d17', '0x4f8c73b2', '0x23d9fe34', '0x17909bd1', '0x74864a54', '0x7120da84', '0x4c238376', '0x28d0791e', '0x154f4ab1', '0x27cd497e', '0x32d48f16', '0x005e68b6', '0x686a839b', '0x3c24c8fb', '0x530722d5', '0x23fe799a', '0x235dc05f', '0x1912a500', '0x5bb5c2b0', '0x5742d7d7', '0x37477606', '0x1d600471', '0x427a8b2c', '0x7d89eccd', '0x378e1019', '0x7c169072', '0x77c59183', '0x1787f375', '0x3888bd46', '0x19f8e75d', '0x2b20ee30', '0x542facf5', '0x15a180f6', '0x70070f20', '0x6c39a0ff', '0x31b37ed9', '0x312b894e', '0x7126476a', '0x1c879970', '0x272cbd95', '0x0183dfe8', '0x67ea7dae', '0x14165ae7', '0x7fc255b2', '0x16c52e94', '0x2145c662', '0x5e1aa808', '0x3504210d', '0x0828c4e4', '0x2820e55d', '0x22e32ac0', '0x66ec39a3', '0x22c09c5e', '0x3d813bf3', '0x03e83fa3', '0x4b4e4247', '0x26889bfb', '0x06665c1a', '0x46bc0107', '0x669b08a0', '0x3418ce6a', '0x02ee61f5', '0x4175c981']

MDS_matrix=[['0x2c51d3f9', '0x480b4bcd', '0x36aa380a'],['0x16a40c67', '0x2c4ff38a', '0x024fab63'],['0x3f2562d6', '0x71321c16', '0x0bccda98']]
...
def perm(input_words):
    ...
    print("\n BEGIN FIRST FULL ROUND \n")
    # First full rounds
    for r in range(0, R_f):
        # Round constants, nonlinear layer, matrix multiplication
        print("First full round index", r)
        print("round constants index",round_constants_round_counter)
        print("round constants",round_constants_field_new[round_constants_round_counter])
        for i in range(0, t):
            state_words[i] = state_words[i] + round_constants_field_new[round_constants_round_counter][i]
        print("full_S_box_no",full_S_box_no)
        full_S_box_no +=1
        for i in range(0, t):
            state_words[i] = (state_words[i])^alpha
        print("full MDS", MDS_matrix_field)
        state_words = list(MDS_matrix_field * vector(state_words))
        round_constants_round_counter += 1

    print("\n BEGIN PRE SPARSE ROUND \n")
    # Middle partial rounds
    # Initial constants addition
    print("Pre presparse round const index",round_constants_round_counter)
    print("round constants",round_constants_field_new[round_constants_round_counter])
    for i in range(0, t):
        state_words[i] = state_words[i] + round_constants_field_new[round_constants_round_counter][i]
    # First full matrix multiplication
    print("Pre sparse matrix",M_i)
    state_words = list(vector(state_words) * M_i)

    print("\n BEGIN PARTIAL ROUND \n")
    for r in range(0, R_P):
        print("\n SParse round number", r)
        # Round constants, nonlinear layer, matrix multiplication
        #state_words = list(vector(state_words) * M_i)
        print("Spartial_S_box_no",partial_S_box_no)
        partial_S_box_no +=1
        state_words[0] = (state_words[0])^alpha
        # Moved constants addition
        if r < (R_P - 1):
            round_constants_round_counter += 1
            print("Pre sparse round const index",round_constants_round_counter)
            print("round constants",round_constants_field_new[round_constants_round_counter])
        ...
        if r == (R_P - 1):
             print("No_round_const_addition")        
        print("SParse round matrix M_0_0", M_0_0)
        print("SParse round matrix: v_collection_index, W_hat_collection_index", R_P - r - 1)
        print("SParse round matrix: v[", R_P - r - 1, "]is", v_collection[R_P - r - 1])
        print("SParse round matrix: w_hat[", R_P - r - 1, "]is", w_hat_collection[R_P - r - 1])
